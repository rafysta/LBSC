...
option_list <- list(  
  make_option(c("-a", "--target"), default="NA", help="Output file from SC.sh for target （A-B's A"),
  make_option(c("-b", "--control"), default="NA", help="Output file from SC.sh for control (A-B's B)"),
  make_option(c("-o", "--out"), help="bedgraph file"),
  make_option(c("--bin"), default="500", help="bin size"),
...
df1 <- fread(FILE_a, skip = 1, header = TRUE) %>% mutate(sample = "target")
df2 <- fread(FILE_b, skip = 1, header = TRUE) %>% mutate(sample = "control")
TOTAL1 <- fread(FILE_a, header = FALSE, nrows = 1, col.names = c("column", "number"))$number
TOTAL2 <- fread(FILE_b, header = FALSE, nrows = 1, col.names = c("column", "number"))$number

#　adjust total score
df1 <- df1 %>% mutate(score = score / TOTAL1 * (CHR_LENGTH / 100))
df2 <- df2 %>% mutate(score = score / TOTAL2 * (CHR_LENGTH / 100))
